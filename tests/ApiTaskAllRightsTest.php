...
class ApiTaskAllRightsTest extends PHPUnit_Framework_TestCase {
    ...
    public function testCreatePermissions(App_CLI $app, Model_User $user)
    {
        ...
        $m
            ->set('user_id',$user['id'])
            ->set('right','can_see_tasks,can_add_task,can_edit_task,can_delete_task,can_add_comment_to_task')
            ->save()
        ;
    ...
    public function testCreateRequirement(
        App_CLI $app, Model_User $user, Model_User_Right $rights, $login_res_success, Model_Project $project, Model_Quote $quote
    ) {
        $this->app = $app;
    ...
    public function testCleanDB(
        ...
        $this->add('Model_Task')->load($task_id)->forceDelete();
