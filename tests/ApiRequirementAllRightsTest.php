<?php
class ApiRequirementAllRightsTest extends PHPUnit_Framework_TestCase {
    ...
    /**
     * Add newly created user all permission for Requirements.
     *
     * @depends testAddApp
    ...
    public function testCreatePermissions(App_CLI $app, Model_User $user)
    {
        ...
        $m
            ->set('user_id',$user['id'])
            ->set('right','can_see_quotes,can_add_requirement,can_edit_requirement,can_delete_requirement,can_add_comment_to_requirement')
            ->save()
        ;
    ...
    public function testCreateProject(
        ...
        $m['name'] ='TestProject_ApiRequirementAllRightsTest_'.$hash;
        $m->save();
    ...
    public function testCreateRequirement(
        ...
        $this->assertEquals($obj->result,'success','Result of creating a Requirement is not successful');

        // obj :: data
        $this->assertObjectHasAttribute('data',$obj,'No data is returned form API after creating a Requirement');
        $this->assertTrue(is_a($obj->data,'stdClass'),'Data is not an object of class stdClass after convertation of API respond on creating a Requirement');

        // obj :: data :: id
        $this->assertObjectHasAttribute('id',$obj->data,'Requirement. Returned data form API doesn\'t have ID');
    ...
    public function testGetRequirement(
        ...
        $url = 'v1/requirement/getById&id='.$create_object_res->data->id.'&lhash='.$user_login_res->hash->lhash;
        ...
        // obj :: data :: id
        $this->assertObjectHasAttribute('id',$obj->data[0],'Requirement. Returned data form API doesn\'t have ID');
    ...
    public function testUpdateRequirement(
        App_CLI $app, Model_User $user, Model_User_Right $rights, $user_login_res, $requirement_create_res
    ) {
        $this->app = $app;

        $hash = time();
        $new_name = 'TestRequirement_'.$hash.'_Updated_'.$hash;
        $url = 'v1/requirement/saveParams&id='.$requirement_create_res->data[0]->id.'&lhash='.$user_login_res->hash->lhash;
        $data = ['name' => $new_name];
        $obj = json_decode($this->do_post_request($url,$data));
        ...
        $this->assertTrue(is_string($obj->result),'Requirement. Result was converted not to string by json_encode()');
        $this->assertEquals($obj->result,'success','Result of updating a Requirement is not successful');

        // obj :: data
        $this->assertObjectHasAttribute('data',$obj,'No data is returned form API after updating a Requirement');
        $this->assertTrue(is_a($obj->data,'stdClass'),'Data is not an object of class stdClass after convertation of API respond on updating a Requirement');

        // obj :: data :: id
        $this->assertObjectHasAttribute('id',$obj->data,'Requirement. Returned data form API doesn\'t have ID');
        // obj :: data :: name
        $this->assertObjectHasAttribute('name',$obj->data,'Requirement. Returned data form API doesn\'t have name field');
        $this->assertTrue( ($obj->data->name==$new_name) ,'Requirement. Name returned by API doesn\'t match setting name');
    ...
    public function testDeleteRequirement(
        App_CLI $app, Model_User $user, Model_User_Right $rights, $user_login_res, $requirement_create_res, $requirement_update_res
    ) {
        $this->app = $app;

        $url = 'v1/requirement/deleteById&id='.$requirement_create_res->data[0]->id.'&lhash='.$user_login_res->hash->lhash;
        $obj = json_decode($this->do_get_request($url));
        ...
        $pr = $this->app->add('Model_Requirement')->load($requirement_create_res->data[0]->id);
    ...
    public function testCleanDB(
        ...
        Model_Project $test_project, Model_Quote $test_quote, $create_requirement_res
        /*$get_requirement_res, $update_requirement_res, $del_requirement_res*/
    ) {

        $this->app = $app;

        $requirement_id = $create_requirement_res->data->id;
        $app->add('Model_Requirement')->load($requirement_id)->forceDelete();
