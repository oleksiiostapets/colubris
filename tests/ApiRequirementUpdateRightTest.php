<?php
class ApiRequirementUpdateRightTest extends PHPUnit_Framework_TestCase {
    ...
    /**
     * Add newly created user some permission to update Requirements only.
     *
     * @depends testAddApp
    ...
    public function testCreatePermissions(App_CLI $app, Model_User $user)
    {
        ...
        $m
            ->set('user_id',$user['id'])
            ->set('right','can_edit_requirement')
            ->save()
        ;
    ...
    public function testUpdateRequirement(
        ...
        $q
            ->set('name','TestRequirement_ApiRequirementUpdateRightTest_'.$hash)
        ...
        $url = 'v1/requirement/saveParams&id='.$q->id.'&lhash='.$user_login_res->hash->lhash;
        $data = ['name' => $new_name];
        $obj = json_decode($this->do_post_request($url,$data));
    ...
    public function testCreateRequirement(
        ...
        $data = [
            'name'     => 'TestRequirement_ApiRequirementUpdateRightTest_'.$hash,
            'quote_is' => $quote->id
        ];
        $obj = json_decode($this->do_post_request($url,$data));
    ...
    public function testGetRequirement(
        ...
        $this->app = $app;
        $this->assertObjectHasAttribute('id',$requirement_res->data,'Saved Requirement doesn\'t have ID');
        $this->assertTrue(!is_null($requirement_res->data->id),'Saved Requirement doesn\'t have ID');

        $url = 'v1/requirement/getById&id='.$requirement_res->data->id.'&lhash='.$user_login_res->hash->lhash;
    ...
    public function testDeleteRequirement(
        ...
        $url = 'v1/requirement/deleteById&id='.$requirement_update_res->data->id.'&lhash='.$user_login_res->hash->lhash;
        $obj = json_decode($this->do_get_request($url));
    ...
    public function testCleanDB(
        ...
        $update_requirement_res_obj
    ) {
        $app->add('Model_Requirement')->load($update_requirement_res_obj->data->id)->forceDelete();
        $quote->forceDelete();
        $project->forceDelete();
        $rights->delete();
        $user->forceDelete();
