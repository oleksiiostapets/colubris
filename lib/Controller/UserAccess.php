...
class Controller_UserAccess extends AbstractController {
    ...
    function whatTaskFieldsUserCanSee($quote=null) {
        if ($this->user->isAdmin()) {
            ...
            return array('quote_id','project','quote','name','priority','type','status','estimate','spent_time','requester','assigned');
        } else if ($this->user->isDeveloper()) {
            return array('quote_id','project','quote','name','priority','type','status','estimate','spent_time','requester','assigned');
        } else if ($this->user->isClient()) {
            if (is_object($quote) && $quote['show_time_to_client']) {
                return array('quote_id','project','quote','name','priority','type','status','estimate','spent_time');
            } else {
                return array('quote_id','project','quote','name','priority','type','status','estimate');
            }
        } else if ($this->user->isSales()) {
            return array('quote_id','project','quote','name','priority','type','status','estimate','spent_time','requester','assigned');
        }
